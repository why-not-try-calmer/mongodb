...
import Control.Concurrent.STM.TChan (TChan, isEmptyTChan, newTChan, readTChan, writeTChan)
import Control.Monad (forM, forever, replicateM, unless)
import Control.Monad.STM (atomically)
import Data.Binary.Get (Get, getInt8, runGet)
import Data.Binary.Put (Put, putInt8, runPut)
...
import Data.Bson (Document, cast, look, merge, valueAt, (=:))
...
data Pipeline = Pipeline
  ...
  -- ^ Mutex on handle, so only one thread at a time can write to it
  , responseQueue :: TChan (MVar (Either IOError Response))
  -- ^ Queue of threads waiting for responses. Every time a response arrives we pop the next thread and give it the response.
...
call pipe notices request = do
 ...
 where
  check requestId (responseTo, reply) =
    if requestId == responseTo
      then reply
      else error $ "expected response id (" ++ show responseTo ++ ") to match request id (" ++ show requestId ++ ")"
