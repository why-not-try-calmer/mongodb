...
import Database.MongoDB.Internal.Network (Host (..), HostName, PortID (..), connectTo, lookupReplicaSetName, lookupSeedList)
import Database.MongoDB.Internal.Protocol (Pipe, close, isClosed, newPipe)
...
readHostPortM = either (fail . show) return . parse parser "readHostPort"
 where
...
updateMembers rs@(ReplicaSet _ vMembers _ _) = do
 ...
 where
  ...
  intersection keys assocs = (partition (flip elem inKeys . fst) assocs, keys \\ inKeys)
   where
